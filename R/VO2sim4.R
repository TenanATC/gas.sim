#' VO2sim Determines the Probability that Two VO2 Measures are the Same Between Testing Days
#'
...
VO2_sim<- function(a, b, system_a='parvo_2400', system_b='parvo_2400', plot = F){
  ...
  if (system_a == 'parvo_2400'){sd1 <- suppressWarnings(predict(VO2_parvo_pred, data.frame(VO2_mean = mu1)))}
  if (system_b == 'parvo_2400'){sd2 <- suppressWarnings(predict(VO2_parvo_pred, data.frame(VO2_mean = mu2)))}
  if (system_a == 'douglas_bag'){sd1 <- suppressWarnings(predict(VO2_douglas_pred, data.frame(VO2_mean = mu1)))}
  if (system_b == 'douglas_bag'){sd2 <- suppressWarnings(predict(VO2_douglas_pred, data.frame(VO2_mean = mu2)))}

  #throw warning that algorithm is extrapolating beyond original data
  if( any(mu1 > 3.85 | mu2 > 3.85) )  warning('Algorithm extrapolating beyond original data, results may be unreliable/unstable')
