#' VCO2sim Determines the Probability that Two VCO2 Measures are the Same Between Testing Days
#'
#' This function models the reliability of VCO2 data from a particular system as univariate normal
...
VCO2_sim<- function(a, b, system_a='parvo_2400', system_b='parvo_2400', plot = F){
  library(ggplot2)
  library(msm)
  library(cubature)
  ...
  if (system_a == 'parvo_2400'){sd1 <- suppressWarnings(predict(VCO2_parvo_pred, data.frame(VCO2_mean = mu1)))}
  if (system_b == 'parvo_2400'){sd2 <- suppressWarnings(predict(VCO2_parvo_pred, data.frame(VCO2_mean = mu2)))}
  if (system_a == 'douglas_bag'){sd1 <- suppressWarnings(predict(VCO2_douglas_pred, data.frame(VCO2_mean = mu1)))}
  if (system_b == 'douglas_bag'){sd2 <- suppressWarnings(predict(VCO2_douglas_pred, data.frame(VCO2_mean = mu2)))}
  
  #throw warning that algorithm is extrapolating beyond original data
  if( any(mu1 > 4.19 | mu2 > 4.19) )  warning('Algorithm extrapolating beyond original data, results may be unreliable/unstable')
