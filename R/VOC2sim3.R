#' VCO2sim Determines the Probability that Two VCO2 Measures are the Same
#'
#' This function models the reliability of VCO2 data from a particular system as univariate normal
#' based on the work by Crouter 2006
#' Additional credit goes to Stackexchange user Wolfgang for his example code of the overlapping coefficient
#'
#' @param a The first VCO2 value obtained
#' @param b The second VCO2 value obtained
#' @param system_a The system used, in quotes, to obtain measurement 'a'.
#' Accepts "parvo_2400" and "douglas_bag". Defaults to parvo_2400.
#' @param system_b The system used, in quotes, to obtain measurement 'b'.
#' Accepts "parvo_2400" and "douglas_bag". Defaults to parvo_2400.
#' @param plot False returns the probability they are same distribution. True returns a plot of overlapping distributions.
#' @export
#' @examples
#' put something here

VCO2_sim<- function(a, b, system_a='parvo_2400', system_b='parvo_2400', plot = F){

  mu1 <- a
  mu2 <- b

  if (system_a == 'parvo_2400'){sd1 <- predict(VCO2_parvo_pred, data.frame(VCO2_mean = mu1))}
  if (system_b == 'parvo_2400'){sd2 <- predict(VCO2_parvo_pred, data.frame(VCO2_mean = mu2))}
  if (system_a == 'douglas_bag'){sd1 <- predict(VCO2_douglas_pred, data.frame(VCO2_mean = mu1))}
  if (system_b == 'douglas_bag'){sd2 <- predict(VCO2_douglas_pred, data.frame(VCO2_mean = mu2))}


  #create funtion for determining integral
  min.f1f2 <- function(x, mu1, mu2, sd1, sd2) {
    f1 <- dnorm(x, mean=mu1, sd=sd1)
    f2 <- dnorm(x, mean=mu2, sd=sd2)
    pmin(f1, f2)
  }




  if (plot == F ){
    #just return integral information
    areacurve <- integrate(min.f1f2, -Inf, Inf, mu1=mu1, mu2=mu2, sd1=sd1, sd2=sd2)
    prob_diff <- areacurve$value*100

    return(prob_diff)
  }else if (plot ==T){
    #Return the simulation data for plotting
    #this needs to be fixed so all the data will plot correctly, just do it later
    xs <- seq(min(mu1 - 3*sd1, mu2 - 3*sd2), max(mu1 + 3*sd1, mu2 + 3*sd2), .001)
    f1 <- dnorm(xs, mean=mu1, sd=sd1)
    f2 <- dnorm(xs, mean=mu2, sd=sd2)
    ys <- min.f1f2(xs, mu1=mu1, mu2=mu2, sd1=sd1, sd2=sd2)
    overall <- data.frame(f1,f2,xs, ys)

    dat_plot <-ggplot(overall, aes(x= xs, y=f1)) +
      geom_line() +
      geom_line(aes(x=xs, y=f2)) +
      geom_ribbon(aes(ymin =0, ymax=ys))

    return(dat_plot)
  }


}
